...
void qr_shift2 ( int n, int p, num_t* Q_ptr, num_t* R_ptr, int i, int j ) {
  ...
  if (i > j) {
    N = i - j;
    for (int l=0; l<n; l++) {
      num_t tempf = R_ptr[l+n*j];
      for (int k=j; k<i; k++) {
        R_ptr[l+n*k] = R_ptr[l+n*(k+1)];
      }
      R_ptr[l+n*i] = tempf;
    }
  } else {
  ...
  for (int k=0; k < N; k++) {
    ...
    givens(R_ptr[j1+n*i1], R_ptr[j1+1+n*i1], G);
    givens_left_apply2(n, p, R_ptr, G, j1, j1+1);
    #ifdef test
    printf("R_c = [");
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < p; j++) {
        printf("%f ", R_ptr[i+n*j]);
      }
      printf(";\n");
    }
    printf("];\n\n");
    ...
    G[1] *= -1; //possibly not the most efficient way to do a transpose...
    G[2] *= -1;
    givens_right_apply2(n, Q_ptr, G, j1, j1+1);
    #ifdef test
    printf("Q_c = [");
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        printf("%f ", Q_ptr[i+n*j]);
      }
      printf(";\n");
    }
    printf("];\n\n");
...
void givens_left_apply2(int n, int p, num_t* A, num_t* G, int row1, int row2) {
  num_t temp;
  for (int i=0; i<p; i++) {
    temp = G[0]*A[row1+n*i] + G[2]*A[row2+n*i];
    A[row2+n*i] = G[1]*A[row1+n*i] + G[3]*A[row2+n*i];
    A[row1+n*i] = temp;
  }
}
...
void givens_right_apply2(int n, num_t* A, num_t* G, int col1, int col2) {
  num_t temp;
  for (int i=0; i<n; i++) {
    temp = A[i+n*col1]*G[0] + A[i+n*col2]*G[1]; 
    A[i+n*col2] = A[i+n*col1]*G[2] + A[i+n*col2]*G[3];
    A[i+n*col1] = temp;
  }
}
